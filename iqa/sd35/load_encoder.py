...
def load_encoder(model, device="cuda", dtype: torch.dtype = torch.bfloat16):
    """
    sample: src_z = vae.encode(image_torch.to(device)).to(device)
            src_z = vae.process_in(src_z).to(device)
    """
...
if __name__ == "__main__":
    ...
    vae_cfg = cfg["vae"]
    model = load_encoder(vae_cfg["vae_pretrained"], device=device, dtype=torch.bfloat16)
    print(model)
