...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.model_mean_type = model_mean_type  # default: gd.ModelMeanType.FLOW_VELOCITY
        self.model_var_type = model_var_type    # default: None
        ...
        # print("="*100)
        # print("Gaussian_Diffusion: use flow:", self.flow)
        # print("="*100)
    ...
    def p_mean_variance(self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None):
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            ...
            print("check model_var_type: predict variance with learnable var.")
            ...
            print("check model_var_type: predict variance with fixed range.")
        else:
            model_variance = th.zeros_like(model_output)
            model_log_variance = th.zeros_like(model_output)
            print("check model_var_type: is None.")
    ...
    def _vb_terms_bpd(self, model, x_start, x_t, t, clip_denoised=True, model_kwargs=None):
        ...
        # generation: model_kwargs=dict(y=y, mask=y_mask, data_info=data_info)
        # ip2p: model_kwargs=dict(c=input_images, y=y, mask=y_mask, data_info=data_info)
        """
    ...
    def training_losses(self, model, x_start, timestep, model_kwargs=None, noise=None, skip_noise=False):
        ...
        # input: model, clean_images, timesteps, model_kwargs=dict(y=y, mask=y_mask, data_info=data_info)
        """
        t = timestep
        if model_kwargs is None:
            model_kwargs = {}
    ...
    def training_losses_p2p(self, model, x_start, timestep, return_output=False, model_kwargs=None, noise=None, skip_noise=False):
        """
        Compute training losses for a single timestep.
        :param model: the model to evaluate loss on.
        :param x_start: the [N x C x ...] tensor of inputs.: target_latent
        :param t: a batch of timestep indices.
        :param model_kwargs: if not None, a dict of extra keyword arguments to
            pass to the model. This can be used for conditioning.
            -> model_kwargs should include (src_latent, text_prompts)
        :param noise: if specified, the specific Gaussian noise to try to remove.
        :return: a dict with the key "loss" containing a tensor of shape [N].
                 Some mean or variance settings may also have other keys.
        
        # input: model, target_images, timesteps, model_kwargs=dict(c=input_images, y=y, mask=y_mask, data_info=data_info)
        """
        t = timestep
        if model_kwargs is None:
            model_kwargs = {}
    ...
    def training_losses_d2c(self, model, x_start, timestep, img_cond=None, return_output=False, model_kwargs=None):
        """
        Compute training losses for a single timestep.
        :param model: the model to evaluate loss on.
        :param x_start: the [N x C x ...] tensor of inputs.: target_latent
        ...
        if img_cond is None:
            raise KeyError("Error: Running DiT on D2C model, need a img_cond inputs as start points!")
        noise = img_cond
        x_t = self.q_sample(x_start, t, noise=noise)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            target = {
                ...
                ModelMeanType.START_X: x_start,
                ModelMeanType.EPSILON: noise,
                ModelMeanType.FLOW_VELOCITY: noise - x_start,  # x_lq - x_hq
            }[self.model_mean_type]
            assert output.shape == target.shape == x_start.shape
            # FLOW_VELOCITY
            if return_output:
            ...
            if model_kwargs.get("mask_ratio", False) and model_kwargs["mask_ratio"] > 0:
                ...
                terms["mse"] = mean_flat(loss) # !!!! current
    ...
    def training_losses_diffusers(self, model, x_start, timestep, model_kwargs=None, noise=None, skip_noise=False):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            output = model(x_t, t, **model_kwargs)[0]
