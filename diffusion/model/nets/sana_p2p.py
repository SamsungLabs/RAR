...
class SanaP2P(Sana):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # p2p concat image condiction, double in_chs.
        self.x_embedder = PatchEmbedMS(patch_size, in_channels*2, hidden_size, kernel_size=kernel_size, bias=True)
        ...
        )  # default: gemmma-2-2b-it, max_tokens=300
    ...
    def forward(self, x, timestep, y, c=None, mask=None, data_info=None, return_logvar=False, jvp=False, **kwargs):
        """
        ...
        # condition
        if c != None:
            x = torch.cat([x, c], dim=1).to(self.dtype)
        else:
            x = torch.cat([x, x], dim=1).to(self.dtype)
            print("# SANA p2p without image condition.")
            # raise TypeError("SANAP2P need image condition inputs.")

        x = self.x_embedder(x) # torch.Size([1, 64, 16, 16]) -> torch.Size([1, 256, 2240])
        ...
        t0 = self.t_block(t)
        y = self.y_embedder(y, self.training, mask=mask)  # (N, D),  torch.Size([1, 1, 300, 2240])
        if self.y_norm:
        ...
        for block in self.blocks:
            ...
            # gradient checkpointing is not supported for JVP
                ...
                x = auto_grad_checkpoint(
                    ...
                    **kwargs,
                    use_reentrant=False,
                )  # (N, T, D) #support grad checkpoint, torch.Size([1, 256, 2240]) BLD

        x = self.final_layer(x, t)  # (N, T, patch_size ** 2 * out_channels),torch.Size([1, 256, 32])
        x = self.unpatchify(x)  # (N, out_channels, H, W), torch.Size([1, 32, 16, 16])
...
#################################################################################
@MODELS.register_module()
def SanaP2P_600M_P1_D28(**kwargs):
    return SanaP2P(depth=28, hidden_size=1152, patch_size=1, num_heads=16, **kwargs)

@MODELS.register_module()
def SanaP2P_1600M_P1_D20(**kwargs):
    # 20 layers, 1648.48M
    return SanaP2P(depth=20, hidden_size=2240, patch_size=1, num_heads=20, **kwargs)