...
def save_weight(
    ...
    if epoch is None:
        ...
        save_path = os.path.join(save_dir, f"ckpt_epoch{epoch}_step{step}.pt")

    torch.save(ckpt, save_path)
    # save tokenizer
    ddp_model.module.assessment.tokenizer.save_pretrained(save_dir)
    # save configuration
    ddp_model.module.assessment.llm.config.save_pretrained(save_dir)
    return save_path
...
def save_checkpoint_ddp(
    ...
    state_dict = dict(state_dict=model_state_dict)
    ...
    # only save trainable parameters
    weight_path = file_path.replace(".pth", "_weight.pth")
    trainable_params = [
        k for (k, v) in model.named_parameters() if v.requires_grad
    ]
    trainable_ckpt = OrderedDict((k, model_state_dict[k]) for k in trainable_params)
    torch.save(dict(state_dict=trainable_ckpt), weight_path)
    if getattr(model, "assessment", None):
        if isinstance(model.assessment, nn.Module):
            model.assessment.tokenizer.save_pretrained(work_dir)
            model.assessment.llm.config.save_pretrained(work_dir)
    logger.info(f"Saved trainable weights of epoch {epoch} to {weight_path}.")

    # whole model with training detail
    ...
    logger.info(f"Saved resume checkpoint of epoch {epoch} to {file_path}.")
    if keep_last:
        ckpt_list = os.listdir(work_dir)
        for previous_ckgt in ckpt_list:
            if previous_ckgt in file_path:
                continue
            if ".pth" in previous_ckgt and "_weight.pth" not in previous_ckgt:
                del_ckpt = os.path.join(work_dir, previous_ckgt)
                if os.path.exists(del_ckpt):
                    os.remove(del_ckpt)
    if add_symlink:
...
def load_checkpoint_ddp(
    ...
    load_weights = {}
    dismatch_weights = {}
    missing_weights = list(model_state_dict.keys())
    for k, v in state_dict.items():
        if k in model_state_dict.keys():
            missing_weights.remove(k)
            if v.shape == model_state_dict[k].shape:
                load_weights[k] = model_state_dict[k]
            else:
                dismatch_weights[k] = [v.shape, model_state_dict[k].shape]

    print("="*100)
    print("missing_weights:", missing_weights)
    print("dismatch_weights:", dismatch_weights)
    print("="*100)

    missing, unexpect = model.load_state_dict(load_weights, strict=False)
    if model_ema is not None:
