    ...
    for epoch in range(start_epoch + 1, config.train.num_epochs + 1):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                loss_term = train_diffusion.training_losses_d2c(
                    model.module.model, target_images, timesteps, img_cond=input_images,
...
def main(cfg: SanaConfig) -> None:
    ...
    DiT.train().to(accelerator.device)
    trainable_params = []
    ...
    if config.connector.load_from:
        ...
        logger.warning(f"Missing keys: {missing}")
        logger.warning(f"Unexpected keys: {unexpected}")
    connector = connector.train().to(connector_dtype).to(accelerator.device)
    trainable_params = []
    ...
    assessment.train().to(accelerator.device)
    trainable_params = []
    ...
    model = IQAIR(DiT, connector, assessment, accelerator.device)
    logger.info("##############################################################")
