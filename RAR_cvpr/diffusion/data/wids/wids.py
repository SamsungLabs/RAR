...
class DistributedWeightedRangedSampler(Sampler):
    ...
    def __init__(
        ...
        if not dist.is_availabe() or not dist.is_initialized():
        ...
        self.worker_end = min(start + self.worker_chunk, num_samples)
        self.local_indices = torch.arange(start, end, dtype=torch.long)
