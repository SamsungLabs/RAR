...
from diffusion.model.sd35.other_impls import Mlp, attention
...
class MMDiTX(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        """
        self.dtype = next(self.x_embedder.parameters()).dtype
        hw = x.shape[-2:]
        ...
        # Time embedding & Text Pooled embedding condition, for AdaZeroLN
        c = self.t_embedder(timestep, dtype=x.dtype)  # (N, D)
        if y is not None:
            y = self.y_embedder(y)  # (N, D)
            c = c + y  # (N, D)
        check_nan("c", c)

        # Text Condition, for cross-attention
        ...
        check_nan("unpatchify", x)
        return x

    def apply_model(self, x, sigma, model_sampling, c_crossattn=None, y=None, skip_layers=[], controlnet_cond=None):
        dtype = self.dtype
...
class MMDiTXP2P(nn.Module):
    ...
    def forward_core_with_concat(
        ...
        for i, block in enumerate(self.joint_blocks):
            ...
            if controlnet_hidden_states is not None:
                ...
                x = x + controlnet_hidden_states[i // controlnet_block_interval]
            check_nan(f"DiT_layer_{i}.x", x)
            # check_nan(f"DiT_layer_{i}.context", context)
        x = self.final_layer(x, c_mod)  # (N, T, patch_size ** 2 * out_channels)
        return x
    ...
    def forward(
        ...
        **kwargs
        ...
        # image condition
        if img_cond != None:
            x = torch.cat([x, img_cond], dim=1).to(self.dtype)
        else:
            x = torch.cat([x, x], dim=1).to(self.dtype)
            print("# MMDiTX-P2P without image condition.")
