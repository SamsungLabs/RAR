...
class SDTokenizer:
    ...
    def tokenize(self, text, return_word_ids=False):
        ...
        # tokenize words
        to_tokenize = (
            unescape_important(text).replace("\n", " ").split(" ")
        )
        tokens = []
        for x in to_tokenize:
            if x != "":
                tokens.append(self.tokenizer(x)["input_ids"][self.tokens_start : -1])
        # reshape token array to CLIP input size
        batched_tokens = []
        batch = []
        if self.start_token is not None:
            batch.append(self.start_token)
        ...
        for i, t_group in enumerate(tokens):
            ...
            while len(t_group) > 0:
                if len(t_group) + len(batch) > self.max_length - 1:
                    ...
                    if is_large:
                        batch.extend(
                            [t for t in t_group[:remaining_length]]
                        )
                    ...
                    # add end token and pad
                        ...
                        if self.pad_to_max_length:
                            batch.extend([pad_token] * (remaining_length))
                    # start new batch
                    batch = []
                    if self.start_token is not None:
                        batch.append(self.start_token)
        ...
        batch.append(self.end_token)
        if self.pad_to_max_length:
            batch.extend([pad_token] * (self.max_length - len(batch)))
        if self.min_length is not None and len(batch) < self.min_length:
            batch.extend([pad_token] * (self.min_length - len(batch)))

        if not return_word_ids:
            batched_tokens = [[t for t in x] for x in batched_tokens]
        return batched_tokens

    def tokenize_batch(self, texts, return_word_ids=False):
        texts = self._ensure_list(texts)
        return [self.tokenize(t, return_word_ids=return_word_ids) for t in texts]
...
class SD3Tokenizer:
    ...
    def tokenize(self, texts):
        ...
        out = {"l":[], "g":[], "t5xxl": []}
        for t in texts:
            out["l"].append(self.clip_l.tokenize(t)[0])
            out["g"].append(self.clip_g.tokenize(t)[0])
            out["t5xxl"].append(self.t5xxl.tokenize(t)[0])
        return out 
