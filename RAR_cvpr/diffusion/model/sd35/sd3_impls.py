...
from diffusion.model.sd35.dit_embedder import ControlNetEmbedder
from diffusion.model.sd35.mmditx import MMDiTX, MMDiTXP2P
...
class P2PModel(torch.nn.Module):
    """Wrapper around the core MM-DiT-P2P model"""
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.diffusion_model = MMDiTXP2P(
            input_size=input_size,
            pos_embed_scaling_factor=None,
            pos_embed_offset=None,
            pos_embed_max_size=pos_embed_max_size,
    ...
    def apply_model(self, x, sigma, c_crossattn=None, y=None, img_cond=None, skip_layers=[], controlnet_cond=None):
        dtype = self.get_dtype()
        timestep = self.model_sampling.timestep(sigma).float()
        controlnet_hidden_states = None
        if controlnet_cond is not None:
        ...
        model_output = self.diffusion_model(
            ...
            context=c_crossattn.to(dtype),
            y=y.to(dtype),
            img_cond=img_cond.to(dtype) if img_cond else None, 
            controlnet_hidden_states=controlnet_hidden_states,
            skip_layers=skip_layers,
        ).float()
        return self.model_sampling.calculate_denoised(sigma, model_output, x)
...
@torch.no_grad()
@torch.autocast("cuda", dtype=torch.float16)
def sample_dpmpp_2m(model, x, sigmas, sampler, extra_args=None):
    """DPM-Solver++(2M)."""
    extra_args = {} if extra_args is None else extra_args
    s_in = x.new_ones([x.shape[0]])
    ...
    old_denoised = None
    for i in tqdm(range(len(sigmas) - 1)):
        denoised = model(x, sigmas[i] * s_in, sampler, **extra_args)
