...
"""
Sample:
    python scripts/test_autocomparison.py \
        --meta_file=./PIR_tar/Dehaze/SOTS/metas/test_iqa_A_brief.json \
        --tag=SOTS \
        --config=configs/infer_cfg.yaml \
        --model_path=checkpoints/RAR_modelzoo/RAR/checkpoints/epoch_5_step_60506_weight.pth \
        --work_dir=output_composite/results/ \
        --data_dir=dataset/PIR_tar/ \
        --sample_nums=300 \
        --resolution=256 \
        --bs=1 \
        --num_workers=3 \
...
if __name__ == "__main__":
    ...
    # metric
    IR_metric = ImageRestorationMetric(metrics=['psnr', 'ssim', 'lpips', 'clipiqa', 'musiq', 'maniqa', 'niqe', 'nima'], nround=args.num_rounds+1, reduce = 'mean', device=device)
    # sampling (args.flow_type {d2c or p2p}, args.mode {online or offline})
    sample_rate = math.ceil(len(loader)/args.save_nums)  
    dit_step = args.step
    num_rounds = args.num_rounds 
    save_idx = 0
    time_encoder_all, time_dit_all, time_decoder_all = 0, 0, 0
    time_iqa_all, time_connect_all = 0, 0
    time_compare_all = 0
    time_data_all, time_eval_all = 0, 0
    ...
    for bidx, batch in enumerate(tqdm(loader)):
        flag = 0
        time_data_all += time.time() - time_data_start
        # if bidx != 1:
        #     continue
        """
        ...
        with torch.no_grad():
            ...
            time_encoder_all += time.time() - time_strat

            hq = torch.clamp((hq + 1.0) / 2.0, min=0.0, max=1.0)
            lq = torch.clamp((lq + 1.0) / 2.0, min=0.0, max=1.0)
            if args.save_result:
                if bidx % sample_rate == 0:
                    save_fname = batch["fname"][save_idx]

                    save_hq_path = os.path.join(gt_img_rt, save_fname)
                    save_hq = 255.0 * rearrange(hq[save_idx], "c h w -> h w c")
                    save_hq = Image.fromarray(save_hq.type(torch.uint8).cpu().numpy())
                    save_hq.save(save_hq_path)
                    
                    save_previous = 255.0 * rearrange(lq[save_idx], "c h w -> h w c")
                    save_previous = Image.fromarray(save_previous.type(torch.uint8).cpu().numpy())

                    curr_meta = meta[save_idx]
                    over_meta = {
                        "distortion_name": curr_meta["distortion_name"],
                        "id": curr_meta["id"],
                        "input": curr_meta["image_A"],
                        "query": curr_meta["conversations"][0]["value"],
                        "result": {}
                    }
            ...
            for n_round in range(num_rounds+1):
                if n_round != 0:            
                    ## Connector (Token Condition)
                    print("index:", bidx, " round:", n_round, " prompt:", prompts, " quality:", output_quality)
                    ...
                    time_connect_all += time.time() - time_connect_start
                    ...
                    time_dit_all += time.time() - time_dit_start
                ...
                time_decoder_all += time.time() - time_decoder_start

                ## [3e]: Eval
                time_eval_start = time.time()
                current_metric = IR_metric.update(recon, hq, n_round)
                time_eval_all += time.time() - time_eval_start
                ...
                time_compare_all += time.time() - time_comp_start

                ## Prepare for next round
                input_images = samples 
                if previous_prompts == prompts and n_round!=0:
                    previous_prompts = ["None"]
                else:
                    previous_prompts = prompts
                if args.mode == "online" or n_round == 0:
                ...
                if args.save_result:
                    if bidx % sample_rate == 0:
                        # save recon image
                        save_recon_path = os.path.join(save_path_dict[n_round], save_fname)
                        save_recon = 255.0 * rearrange(recon[save_idx], "c h w -> h w c")
                        save_recon = Image.fromarray(save_recon.type(torch.uint8).cpu().numpy())
                        save_recon.save(save_recon_path)
                        # save concate results
                        save_cat_path = os.path.join(save_path_dict['result'], "".join(save_fname.split(".")[:-1])+f"_n{n_round}.png")
                        save_cat_img = concat_horizontal([save_previous, save_recon, save_hq])
                        save_cat_img.save(save_cat_path)
                        # change previous image
                        save_previous = save_recon

                        # prepare single meta
                        curr_meta = meta[save_idx]
                        curr_meta["image_B"] = curr_meta["image_A"]
                        curr_meta["image_A"] = os.path.relpath(save_recon_path, start=save_path_dict["work_dir"])
                        if len(curr_meta["conversations"]) == 3:
                            curr_meta["conversations"][2] = {
                                "from": "qa",
                                "value": output_texts[save_idx],
                                "output_id": None,
                                "prob": None,
                                "confidence": confidences[save_idx],
                            }
                        else:
                            curr_meta["conversations"].append({
                                "from": "qa",
                                "value": output_texts[save_idx],
                                "output_id": None,
                                "prob": None,
                        ...
                        curr_meta["quality_comparison"].append({
                                "from": "qa",
                                "value": output_quality[save_idx],
                                "confidence": confid_quality[save_idx],
                            })
                        single_meta[n_round].append(curr_meta)
                        # prepare overall meta 
                        over_meta["result"][n_round] = {
                            "Metric": "".join([f"{k}: {v}, " for k, v in current_metric.items()]),
                            "QA": output_texts[save_idx],
                            "Confidence": confidences[save_idx],
                            "Comparison": output_quality[save_idx],
                            "Comp_Confidence": confid_quality[save_idx]
                        }

                if n_round != 0:
                    if "Image B" in output_quality[0] or n_round == num_rounds:
                        if flag == 0:
                            for m, v in previous_metric.items():
                                IR_metric.metric_results[m].append(v)
                            # print(IR_metric.metric_results)
                            record_rounds.append(n_round-1)
                            flag = 1
                            break
            ...
            if args.save_result:
                if bidx % sample_rate == 0:
                    detail_meta.append(over_meta)
        time_data_start = time.time()

    logger.info("====================================================================================================")
    for word, value in model.assessment.dictionary.items():
        if "\n" in word:
            word = word.replace("\n", "")
    ...
    if args.save_result:
        # save single meta
        for n_round in range(num_rounds+1):
            with open(os.path.join(save_path_dict["root"], f"{n_round}.json"), "w") as f:
                json.dump(single_meta[n_round], f, indent=3)

        # save overall meta
        with open(os.path.join(save_path_dict["root"], "detail.json"), "w") as f:
            json.dump(detail_meta, f, indent=2)

    average_round = np.mean(record_rounds)
    logger.info("Time:")
    logger.info("====================================================================================================")
    logger.info(f"Total: Encoder: {time_encoder_all}s, IQA: {time_iqa_all}s, Compare: {time_compare_all}s, Connector: {time_connect_all}s,  DiT: {time_dit_all}s, Decoder: {time_decoder_all}s, Eval: {time_eval_all}s, Overall: {time_encoder_all+time_iqa_all+time_compare_all+time_connect_all+time_dit_all+time_decoder_all}")
    ## Calculate final results
    time_encoder_all, time_iqa_all, time_connect_all = time_encoder_all / len(loader),  time_iqa_all / len(loader), time_connect_all / len(loader)
    time_compare_all, time_dit_all, time_decoder_all, time_eval_all = time_compare_all / len(loader), time_dit_all / len(loader), time_decoder_all / len(loader), time_eval_all / len(loader)
    logger.info(f"Encoder: {time_encoder_all}/{time_encoder_all}s, IQA: {time_iqa_all/(average_round+1)}/{time_iqa_all}s, Connector: {time_connect_all/(average_round+1)}/{time_connect_all}s,  DiT: {time_dit_all/(average_round+1)}/{time_dit_all}s, Decoder: {time_decoder_all/(average_round+1)}/{time_decoder_all}s, Eval: {time_eval_all/(average_round+1)}/{time_eval_all}s")
    logger.info("====================================================================================================")
    logger.info("Final Results:")
    logger.info("====================================================================================================")
    ir_text = ""
    Fianl_results = IR_metric.results()
    for m, v in Fianl_results.items():
        ir_text += f"{v} "
    logger.info(f"{ir_text}")
    logger.info(f"Total Rounds: {np.sum(record_rounds)}, Average Rounds: {np.mean(record_rounds)}")
    logger.info("====================================================================================================")
    logger.info("DiT Results:")
    logger.info("====================================================================================================")
    metric_label = " ".join(IR_metric.metrics)
    logger.info(f"{metric_label}")  
    for n_round in range(num_rounds+1):
        time_start = time.time()
        IR_results = IR_metric.compute(n_round)
        time_compute = time.time()
        ir_text = ""
        for m, v in IR_results.items():
            ir_text += f"{v} "
        logger.info(f"{ir_text}")
        time_log = time.time()
        IR_metric.reset(n_round)
    logger.info("====================================================================================================")
